...
struct sts_params {
    ...
    std::string vad_model;          // Silero VAD ggml model; empty => energy VAD
    float vad_threshold = 0.0f;     // 0 => wrapper default (0.5 Silero / 0.012 energy)
};
...
static void print_usage(const char * argv0) {
    fprintf(stderr,
        ...
        "      --vad-model F  Silero VAD ggml model for --stream (default: energy VAD)\n"
        "      --vad-thold X  VAD threshold: prob (Silero) / RMS (energy)  (default: auto)\n\n",
...
static bool parse_args(int argc, char ** argv, sts_params & p) {
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--vad-model")    p.vad_model = next("--vad-model");
        else if (a == "--vad-thold")    p.vad_threshold = std::stof(next("--vad-thold"));
...
static int run_stream_demo(const sts_params & p, const std::vector<float> & pcm16) {
    ...
    wp.vad_model   = p.vad_model.empty() ? nullptr : p.vad_model.c_str();
    if (p.vad_threshold > 0.0f) wp.vad_threshold = p.vad_threshold;
    ...
    printf("vad        : %s\n", p.vad_model.empty()
           ? "energy (RMS)" : ("Silero  " + p.vad_model).c_str());
