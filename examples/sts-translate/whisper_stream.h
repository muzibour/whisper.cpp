...
typedef struct whisper_stream_params {
    ...
    // VAD endpointing. Two backends:
    //   • Silero  (neural) — set vad_model to a ggml Silero VAD model; far more
    //     robust to noise/music than energy. Streamed via the LSTM-stateful
    //     whisper_vad_detect_speech_no_reset over 32 ms windows. PREFERRED.
    //   • energy  (RMS)    — fallback when vad_model is NULL or fails to load.
    const char * vad_model;    // path to Silero VAD ggml model (NULL = energy fallback)
    float vad_threshold;       // speech prob (Silero) / RMS energy (fallback) threshold
                               //   Silero default 0.5, energy default 0.012 — see defaults
    int   vad_frame_ms;        // energy-VAD frame granularity (ignored by Silero) (default 20)
