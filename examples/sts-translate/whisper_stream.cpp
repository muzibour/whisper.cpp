...
// Silero's detect_speech logs 4 INFO lines per call; at ~30 calls/s that buries
// the transcript. Once models are loaded we install this filter to pass only
// warnings/errors through (model-load + backend-init logs already happened).
void quiet_log_cb(enum ggml_log_level level, const char * text, void * /*user*/) {
...
// Silero VAD window @ 16 kHz: 512 samples = 32 ms. Fixed by the whisper.cpp
// Silero model (the loader supports only this geometry); detect_speech consumes
// the input in chunks of this size and emits one speech-probability per chunk.
static const int SILERO_WINDOW_N = 512;

struct whisper_stream {
    whisper_stream_params p{};
    whisper_context * ctx = nullptr;
    whisper_vad_context * vctx = nullptr;   // Silero VAD; NULL => energy fallback
    Resampler rs;
    ...
    int64_t vad_processed = 0;    // absolute index up to which VAD has consumed audio
    ...
    void finalize(int64_t utt_end) {
        ...
        // fresh LSTM state for the next utterance (Silero is stateful across calls)
        if (vctx) whisper_vad_reset_state(vctx);
    }

    // Advance the endpointing state machine by one VAD frame, given whether that
    // frame was classified as speech. Shared by both the Silero and energy paths.
    void apply_voiced(bool voiced, int64_t frame_n) {
        if (voiced) {
            if (!in_utt) { in_utt = true; utt_start = vad_processed; last_partial_at = vad_processed; }
            silence_run = 0;
        } else if (in_utt) {
            silence_run += frame_n;
        }
        vad_processed += frame_n;
    }

    // Neural VAD: stream the unconsumed tail through Silero in whole 32 ms windows,
    // carrying LSTM state across calls (no_reset). One speech-prob per window.
    void run_vad_silero() {
        const int W = SILERO_WINDOW_N;
        const int nwin = (int) ((abs_end() - vad_processed) / W);
        if (nwin <= 0) return;
        const float * src = audio.data() + (vad_processed - base);
        if (!whisper_vad_detect_speech_no_reset(vctx, src, nwin * W)) return;
        const int     np = whisper_vad_n_probs(vctx);
        const float * pr = whisper_vad_probs(vctx);
        for (int i = 0; i < np; ++i) apply_voiced(pr[i] >= p.vad_threshold, W);
    }

    // Fallback VAD: RMS energy over fixed frames of the unconsumed tail.
    void run_vad_energy() {
        while (vad_processed + vad_frame_n <= abs_end()) {
            const float * f = audio.data() + (vad_processed - base);
            double sq = 0.0;
            for (int k = 0; k < vad_frame_n; ++k) sq += (double) f[k] * f[k];
            apply_voiced(std::sqrt(sq / vad_frame_n) >= p.vad_threshold, vad_frame_n);
        }
    }
    ...
    void ingest(const std::vector<float> & chunk) {
        ...
        if (vctx) run_vad_silero(); else run_vad_energy();

        // endpoint: enough trailing silence after speech, or utterance too long.
        // Boundaries are measured against the VAD frontier, not raw audio end.
        if (in_utt && (silence_run >= endpoint_n ||
                       (vad_processed - utt_start) >= max_utt_n)) {
            finalize(vad_processed - silence_run);
        }
...
whisper_stream_params whisper_stream_default_params(void) {
    ...
    p.vad_model = nullptr;          // NULL => energy fallback below
    p.vad_threshold = 0.012f;       // energy default; init() bumps to 0.5 for Silero
    p.vad_frame_ms = 20;
    return p;
}
...
whisper_stream * whisper_stream_init(const whisper_stream_params * params) {
    ...
    // Optional neural VAD. On failure we log and fall back to energy (vctx stays NULL).
    if (params->vad_model && params->vad_model[0]) {
        whisper_vad_context_params vp = whisper_vad_default_context_params();
        // Keep the VAD on CPU even when STT runs on GPU: the Silero graph trips a
        // CUDA backend assertion ("pre-allocated tensor ... cannot run the operation")
        // in this whisper.cpp, and it's a 0.88 MB LSTM over 32 ms windows — negligible
        // on CPU and off the STT decode's critical path.
        vp.use_gpu    = false;
        vp.n_threads  = params->n_threads;
        s->vctx = whisper_vad_init_from_file_with_params(params->vad_model, vp);
        if (!s->vctx) {
            std::fprintf(stderr, "whisper_stream: VAD model '%s' failed to load; "
                                 "falling back to energy VAD\n", params->vad_model);
        } else if (s->p.vad_threshold <= 0.05f) {
            // Caller left the energy-scale default; use a sane Silero probability.
            s->p.vad_threshold = 0.5f;
        }
        // Silence the per-window VAD INFO spam now that models are loaded.
        whisper_log_set(quiet_log_cb, nullptr);
    }
...
void whisper_stream_free(whisper_stream * s) {
    if (!s) return;
    if (s->vctx) whisper_vad_free(s->vctx);
